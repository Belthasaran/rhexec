...
test('boot-restore embeds VRAM/CGRAM/OAM/ARAM and pokes NMI + INIDISP before JML', () => {
  ...
  assert.ok(findSeq(stub, [0xa9, 0x00, 0x8f, 0x42, 0x21, 0x00]) >= 0, 'IPL dest low $0000 (32KiB)');
  assert.ok(findSeq(stub, [0xa9, 0x00, 0x8f, 0x43, 0x21, 0x00]) >= 0, 'IPL dest high $0000 (32KiB)');
  assert.equal(countSeq(stub, [0xc0, 0x00, 0x00]), 1, 'high copier stream allows handshake 0');
  assert.equal(countSeq(stub, [0xc0, 0x00, 0x80]), 1, 'IPL low 32KiB CPY #$8000');
  ...
  const lowKick = iplNextCommandKick(copier.length);
  const jumpKick = iplNextCommandKick(0);
  assert.ok(lowKick !== 0 && lowKick !== 1, 'dest-$0000 leftover $2140 is not a skip-0 index');
  assert.equal(jumpKick, 2, 'after 32KiB IPL Y=0 so jump kick is 2');
  assert.ok(findSeq(stub, [0xa9, lowKick, 0x8f, 0x40, 0x21, 0x00]) >= 0, 'dest-$0000 kick is plant Y+2');
  assert.ok(findSeq(stub, [0xa9, jumpKick, 0x8f, 0x40, 0x21, 0x00]) >= 0, 'jump kick is 2 after 32KiB');
  ...
  assert.ok(findSeq(copier, [0x8f, 0x80, 0x01]) >= 0, 'copier dest starts $8000');
  assert.equal(findSeq(copier, [0x68, 0xf0]), -1, 'page0 MMIO skip not needed on high half');
