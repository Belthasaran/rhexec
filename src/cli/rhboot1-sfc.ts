...
async function main(argv: string[]): Promise<number> {
  ...
  // #region agent log
  fetch('http://localhost:7700/ingest/a16a51ec-9c44-41df-b5a8-3a0cdb17c431', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': 'c4b0c8' }, body: JSON.stringify({ sessionId: 'c4b0c8', hypothesisId: 'H', location: 'rhboot1-sfc.ts:main', message: 'wrote boot sfc', data: { out, romBytes: built.rom.length, stubOffset: built.stubOffset, pkg: '0.2.43' }, timestamp: Date.now(), runId: 'post-fix-043' }) }).catch(() => {});
  // #endregion
  return 0;
}
