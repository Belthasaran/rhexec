...
- `rhboot1-sfc` — embed WRAM/VRAM/CGRAM/OAM/ARAM after a reset trampoline; timed IPL (32KiB then 256-byte high pages) plus `$FF80` SPC resume trampoline (no-`$AA` skip jumps IPL to STOP there); restore INIDISP and `$4200` before jumping to the captured PC
