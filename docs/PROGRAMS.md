...
Technique A: new SFC with reset trampoline plus WRAM/VRAM/CGRAM/OAM/ARAM payload. Re-enables NMI (`$4200`) and unblanks INIDISP. SPC IPL plants a copier at `$FF80`, streams the low 32KiB (dest `$0000`), then Start-jumps the copier which copies `$8000–$FF7F` (index handshake, skip 0 after wrap) and `JMP`s `$0386`. The trampoline restores CONTROL/DP, copies DSP regs, write-triggers KON from captured ENVX, then `JMP`s the aligned captured PC. No-`$AA` skip jumps IPL to STOP at `$0386`. Does not patch `$05D89B`/`$05DCDD`. SA-1 is not restored.
