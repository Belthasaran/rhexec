...
Proof-of-concept **SNES execution-state** capture and playback. Standalone from the RHPlay Electron app: capture an unpatched ROM in Mesen 2 as portable `.rhstate1`, then materialize Technique A (boot-restore SFC), B (sd2snes YAML), C (core-native savestate files), or D (seamless launch of the original ROM).
...
npm run rhlaunch1-bizhawk -- --rom game.sfc --state game.rhstate1 --connector Connector.lua
npm run rhlaunch1-mercury -- --rom game.sfc --state game.rhstate1
npm run rhstate1-mss -- --rom game.sfc --state game.rhstate1
npm run rhstate1-bizhawk -- --rom game.sfc --state game.rhstate1
npm run rhstate1-mercury -- --rom game.sfc --state game.rhstate1
```
...
Live Mesen capture is skipped unless `MESEN_PATH` points at a real binary. BizHawk/RetroArch GUI launches are not spawned by `npm test`. Mercury serialize tests skip without `MERCURY_CORE`.
